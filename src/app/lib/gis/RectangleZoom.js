...
(function() {
    ...
    var RectangleZoom =
        ...
        BMapLib.RectangleZoom = function(map, opts){
            ...
            this._opts = {
                ...
                // 开启拉框缩放状态后，鼠标跟随的文字
                followText : "",
                followTextStyle: {color : "#333", borderColor : "#7ABCE9"},
                // 遮盖层外框的线宽
                strokeWeight : 2,
                // 遮盖层外框的颜色
                strokeColor : "#111",
            ...
            this._operationMask = new OperationMask();
        }
    ...
    RectangleZoom.prototype.setCursor = function(cursor) {
        ...
        // 设置鼠标样式
        this._operationMask.setCursor(this._opts.cursor);
    };
    ...
    RectangleZoom.prototype._bind = function(){
        ...
        addEvent(this._map.getContainer(), "mousemove", function(e){
            ...
            var pt = me._operationMask.getDrawPoint(e, true);
    ...
    RectangleZoom.prototype.open = function() {
        ...
        var beginDrawRect = function(e) {        
            ...
            // 增加IE浏览器下对事件的捕获
            if (!!ieVersion && me._operationMask.getDom().setCapture) {
                me._operationMask.getDom().setCapture();
            }
            ...
            addEvent(me._operationMask.getDom(), "mousemove", drawingRect);
            addEvent(me._operationMask.getDom(), "mouseup", endDrawRect);

            // 记录此时鼠标相关位置
            me._bind.mx = e.offsetX || 0;
            me._bind.my = e.offsetY || 0;
            me._bind.ix = e.pageX || e.clientX || 0;
            me._bind.iy = e.pageY || e.clientY || 0;

            // 创建矩形半透明效果框
            insertHTML(me._operationMask.getDom(), "beforeBegin", me._generateHTML());
            me._fDiv = me._operationMask.getDom().previousSibling;
        ...
        var endDrawRect = function(e) {
            if (me._isOpen == true) {
                ...
                // 释放IE浏览器下对事件的捕获
                if (!!ieVersion && me._operationMask.getDom().releaseCapture){
                    me._operationMask.getDom().releaseCapture();
                }
                ...
                // 设置鼠标移动过程中，跟随的文字提示框的位置
                var pt = me._operationMask.getDrawPoint(e);
                if (me._followTitle) {
                    me._followTitle.setPosition(pt);
                    me._followTitle.show();
                }
        ...
        this._operationMask.show(this._map);
        this.setCursor(this._opts.cursor);

        // 增加鼠标按下时，开始绘制矩形框的事件
        // 通过判断只绑定一次，并不再删除
        if (!this._isBeginDrawBinded) {
            addEvent(this._operationMask.getDom(), "mousedown", beginDrawRect);
    ...
    function OperationMask () {
         ...
         * @type {String}
         */
        this._html = "<div style='background:transparent url(https://api.map.baidu.com/images/blank.gif);position:absolute;left:0;top:0;width:100%;height:100%;z-index:1000' unselectable='on'></div>";
     ...
     * @return 无返回值
     */
    OperationMask.prototype.show = function(map) {
        if (!this._map) {
            this._map = map;
        }
        this._inUse = true;
     ...
     * @return 无返回值
     */
    OperationMask.prototype._createMask = function(map) {
        this._map = map;
        if (!this._map) {
            return;
        }
     ...
     * @return Point对象的位置信息
     */
    OperationMask.prototype.getDrawPoint = function(e, n) {
        e = window.event || e;
        var x = e.offsetX || 0;
        var y = e.offsetY || 0;
        var t = e.target || e.srcElement;
        if (t != this.getDom() && n == true) {
     ...
     * @return 无返回值
     */
    OperationMask.prototype.hide = function() {
        if (!this._map) {
            return;
        }
        this._inUse = false;
     ...
     * @return HTML容器元素
     */
    OperationMask.prototype.getDom = function() {
        if (!this._maskElement) {
            this._createMask(this._map);
        }
     ...
     * @return 无返回值
     */
    OperationMask.prototype.setCursor = function(cursor) {
