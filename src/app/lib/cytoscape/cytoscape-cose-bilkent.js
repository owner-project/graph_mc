...
Layout.prototype.getFlatForest = function () {
  ...
  return {
    forest: isForest ? flatForest : [],
    components: flatForest
  };
};
...
FDLayout.prototype.initSpringEmbedder = function () {
  ...
  if (!isNaN(+FDLayoutConstants.options.numIter)) {
    this.maxIterations = Math.max(1, FDLayoutConstants.options.numIter);
  }
  else {
  ...
  // console.log(this.getAllNodes().length, this.maxIterations)
...
FDLayout.prototype.isConverged = function () {
  ...
  //console.log(converged, this.totalDisplacement, this.totalDisplacementThreshold, this.getAllNodes().length)
...
function _CoSELayout(_options) {
  FDLayoutConstants.options = _options;
...
CoSELayout.prototype.classicLayout = function () {
  ...
  if (!this.incremental) {
    travelData = this.getFlatForest();
    var forest = travelData.forest;
    var components = travelData.components;
...
//console.log(this.maxIterations, this.totalIterations, this.isGrowthFinished, this.prunedNodesAll)
...
CoSELayout.prototype.separateComponents = function (components) {
  var boxlist = [];
  ...
  boxlist[0] = this.getComponentBoundingBox(components[0]);
  ...
  for (var i = 1; i < components.length; i++) {
    ...
    var box = this.getComponentBoundingBox(component);
    ...
    this.translateNodes(component, offset);

    boxlist[i] = this.getComponentBoundingBox(component);
    lineBox.push(boxlist[i]);
  }

}

CoSELayout.prototype.translateNodes = function (nodes, offset) {
  for (var i = 0; i < nodes.length; i++) {
    var rect = nodes[i].rect;

    rect.setX(rect.getX() + offset.x);
    rect.setY(rect.getY() + offset.y);
  }
}

CoSELayout.prototype.getComponentBoundingBox = function (component) {
  var height = 0;
  var width = 0;
  var x = Infinity;
  var y = Infinity;
